...
export async function POST(request: Request) {
  ...
  try {
    ...
    const code = error instanceof CloudTrashPurgeError ? error.code : "purge_failed";
  ...
  try {
    await requestCloudTrashPurge(actor.userId, values.trashEntryId);
    await runImmediateTrashWorker("purge");
    return noStoreJson(
      { ok: true, action: "purging", status: "accepted" },
      202,
    );
  } catch (error) {
    const status = error instanceof CloudTrashPurgeError ? error.statusCode : 500;

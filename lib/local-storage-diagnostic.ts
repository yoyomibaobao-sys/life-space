...
export async function logLifespaceStorageDiagnostic(
  ...
  const identityUserIds = identityCacheUserIds();
  const recoveredIdentity = recoverStoredOwnerFromIdentityCache();
  const counts = {
    archives: 0,
    localProject: 0,
    savedLocalCopy: 0,
  ...
  const payload = {
    ...
    identityCacheUserIds: identityUserIds,
    identityCacheUserId: recoveredIdentity?.userId || identityUserIds[0] || null,

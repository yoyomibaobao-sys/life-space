...
test("release signing is environment-only and local records are excluded from Android backup", () => {
  ...
  assert.match(gradle, /ANDROID_VERSION_CODE'\) \?: '24'/);
  assert.match(gradle, /ANDROID_VERSION_NAME'\) \?: '1\.0\.4-rc17'/);
  assert.match(workflow, /ANDROID_VERSION_CODE: \$\{\{ inputs\.version_code \|\| '24' \}\}/);
  assert.match(workflow, /ANDROID_VERSION_NAME: \$\{\{ inputs\.version_name \|\| '1\.0\.4-rc17' \}\}/);

...
test("release signing is environment-only and local records are excluded from Android backup", () => {
  ...
  assert.match(gradle, /ANDROID_VERSION_CODE'\) \?: '25'/);
  assert.match(gradle, /ANDROID_VERSION_NAME'\) \?: '1\.0\.4-rc18'/);
  assert.match(workflow, /ANDROID_VERSION_CODE: \$\{\{ inputs\.version_code \|\| '25' \}\}/);
  assert.match(workflow, /ANDROID_VERSION_NAME: \$\{\{ inputs\.version_name \|\| '1\.0\.4-rc18' \}\}/);

...
test("Android shell keeps its identity, HTTPS host, and offline fallback explicit", () => {
  ...
  assert.match(offlineSource, /offlineMode: "Local offline mode"/);
...
test("Android login uses one resize path and blocks IME extracted-text overlays", () => {
  ...
  const mobileNavigation = read("components/mobile/MobileBottomNavigationView.tsx");
  const globals = read("app/globals.css");
  ...
  assert.match(mobileNavigation, /data-mobile-bottom-nav="true"/);
  assert.match(
    globals,
...
test("release signing is environment-only and local records are excluded from Android backup", () => {
  ...
  assert.match(gradle, /ANDROID_VERSION_CODE'\) \?: '20'/);
  assert.match(gradle, /ANDROID_VERSION_NAME'\) \?: '1\.0\.4-rc13'/);
  assert.match(workflow, /ANDROID_VERSION_CODE: \$\{\{ inputs\.version_code \|\| '20' \}\}/);
  assert.match(workflow, /ANDROID_VERSION_NAME: \$\{\{ inputs\.version_name \|\| '1\.0\.4-rc13' \}\}/);

...
test("release signing is environment-only and local records are excluded from Android backup", () => {
  ...
  assert.match(gradle, /ANDROID_VERSION_CODE'\) \?: '23'/);
  assert.match(gradle, /ANDROID_VERSION_NAME'\) \?: '1\.0\.4-rc16'/);
  assert.match(workflow, /ANDROID_VERSION_CODE: \$\{\{ inputs\.version_code \|\| '23' \}\}/);
  assert.match(workflow, /ANDROID_VERSION_NAME: \$\{\{ inputs\.version_name \|\| '1\.0\.4-rc16' \}\}/);

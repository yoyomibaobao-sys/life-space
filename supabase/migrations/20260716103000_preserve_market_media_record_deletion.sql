-- Preserve shared market media paths when their source record is deleted.
-- The source record is auto-filled only for new or newly selected media;
-- foreign-key cleanup may set it to null without the trigger restoring it.
  ...
  IF new.source_media_id IS NOT NULL THEN
    ...
    IF new.source_record_id IS NULL THEN
      ...
      ELSIF TG_OP = 'UPDATE' THEN
        IF old.source_media_id IS DISTINCT FROM new.source_media_id THEN
          new.source_record_id := v_media.record_id;
        END IF;
      END IF;
    END IF;
    ...
    IF new.source_record_id IS NOT NULL
       AND new.source_record_id <> v_media.record_id THEN
...
'Validates market media ownership and trusted paths while allowing foreign keys to clear source links during record deletion.';

...
const text = {
  zh: {
    mySpace: "????", settings: "??", language: "??", all: "??", cloud: "???", local: "??", project: "??",
    home: "??", follow: "??", market: "??", me: "?", guides: "??", discover: "??", experience: "??",
    cloudUnavailable: "??????????????", cloudProjects: "????", cloudLoading: "?????????",
    cloudLoadFailed: "???????????????", cloudSignIn: "??????????",
    saveLocalCopy: "?????", refreshLocalCopy: "??????", openLocalCopy: "??????",
    offlineCopies: "??????", cacheNotReady: "??????????????????????", cloudCacheReadOnly: "???????????????????????????", noCachedProjects: "??????????????????????????", 
    savingCloudCopy: "????????", cloudCopySaved: "??????????",
    pendingUpload: "???????????????", uploadNow: "????", later: "??",
    uploading: "?????", uploadSuccess: "???????", uploadFailed: "?????????????",
    login: "??", logout: "????", email: "??", password: "??", loginFailed: "????", captchaRequired: "????????",
    camera: "??", album: "?????", chooseProject: "????",
    guideSearch: "??????", guideHint: "???????????????", details: "??", properties: "??",
    guideOverview: "????", basicReferences: "????", createFromGuide: "????????",
    guideOfflineNotice: "??????????????????????????????",
    guideSignInRequired: "????????????????", guideUnavailable: "????????????",
    ongoing: "???", ended: "???", period: "????", enablePeriod: "????", periodDate: "??????", status: "????", visibility: "????", private: "?????",
    photoLimit: "??????10????????", removePhoto: "????",
    brand: "?????",
    offlineMode: "??????",
    offlineTitle: "???????????",
    offlineBody: "?????????????????????????????????????",
    migrationWarning: "?????????????????????????????? App ???",
    reconnect: "??????",
    newProject: "????",
    localProjects: "????",
    noProjects: "???????",
...
function App() {
  ...
  const useCloudCacheSource = !online;
  ...
  function activityProjectView(item: DiscoveryProjectFeedItem) {
    ...
    return {
      ...
      visibilityLabel: language === "zh" ? "??" : "Public",
    ...
    <main className="offline-shell">
      ...
      {screen.kind === "list" ? (
        <PersonalSpaceMobileIdentity
          ...
          experienceLabel={language === "zh" ? "???" : "Experience"}
      ...
      {screen.kind === "activity" ? <>
        ...
        {!online ? (
          ...
          <section className="panel empty">
            {language === "zh" ? "?????????" : "Loading public activity?"}
          </section>
        ) : activityError ? (
          <section className="notice warning">
            <p>{language === "zh" ? "???????????????" : "Could not load public activity."}</p>
            <div className="action-row">
          ...
          <section className="panel empty">
            {language === "zh" ? "?????????" : "No public activity yet."}
          </section>
        )}
      </> : null}
      ...
      {screen.kind === "experience" ? <>
        ...
        {!online ? (
          ...
          <section className="panel empty">
            {language === "zh" ? "???????" : "Loading experience?"}
          </section>
        ) : experienceError ? (
          <section className="notice warning">
            <p>{language === "zh" ? "?????????????" : "Could not load experience."}</p>
            <div className="action-row">
          ...
          <section className="panel empty">
            {language === "zh" ? "?????????" : "No public experience yet."}
          </section>
        )}
      </> : null}
      ...
      {screen.kind === "following" ? (
        !online ? (
          ...
          <>
            ...
            {followedLoading ? (
              <section className="panel empty">
                {language === "zh" ? "?????????" : "Loading followed projects?"}
              </section>
            ) : followedError ? (
              <section className="notice warning">
                <p>{language === "zh" ? "???????????????" : "Could not load followed projects."}</p>
                <div className="action-row">
              ...
              <section className="panel empty">
                {language === "zh" ? "???????????" : "No followed public projects yet."}
              </section>
            )}
          </>
        )
      ...
      {screen.kind === "market" ? (
        !online ? (
          ...
          <section className="panel empty">
            {language === "zh" ? "???????" : "Loading market?"}
          </section>
        ) : marketError ? (
          <section className="notice warning">
            <p>{language === "zh" ? "?????????????" : "Could not load the market."}</p>
            <div className="action-row">
          ...
          <>
            <div className="category-row" role="group" aria-label={language === "zh" ? "????" : "Market type"}>
              {[{ value: "all", label: language === "zh" ? "??" : "All" }, ...getMarketPostTypeOptions(language)].map((option) => (
            ...
            {marketFiltersOpen ? <div className="field">
              ...
              <label>{language === "zh" ? "??" : "Area"}<input value={marketLocationFilter} onChange={(event) => setMarketLocationFilter(event.target.value)} /></label>
              <label>{language === "zh" ? "??" : "Content"}<input value={marketContentFilter} onChange={(event) => setMarketContentFilter(event.target.value)} /></label>
            </div> : null}
            <div className="section-title">
              <h1>{copy.market}</h1>
              ...
              {visibleMarketItems.map((item) => {
                ...
                return (
                  <article style={mobileMarketCardStyle} key={item.id}>
                    <MobileMarketFeedCard item={item} profile={profile} archive={archive} language={language} marketName={copy.market} unsetUsername={language === "zh" ? "??????" : "Unknown user"} notProvided={language === "zh" ? "???" : "Not provided"} />
                  </article>
                );
              })}
            </div> : <section className="panel empty">{language === "zh" ? "????????????" : "No matching posts."}</section>}
          </>
        )
      ) : null}
...
function ProjectForm({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (category === "plant" && (!archive || archive.planting_region || hasRegionDraft) && !normalizedRegion) {
      setError(language === "en" ? "Enter the planting country and city / district." : "???????????????????");
      return;
    }
    setBusy(true);
    setError("");
  ...
  return <>
    ...
    {isCloudCache ? (
      <p className="project-meta">
        {archiveCopy.cloud_offline_cache} / {archiveCopy.cloud_offline_cache_readonly}
      </p>
    ) : null}
    ...
    {tab === "properties" ? <>
      ...
      <div className="record-list">{detail.records.filter((record) => !archive.cycle_enabled || filter === "all" || (filter === "none" ? !record.cycle_id : record.cycle_id === filter)).map((record) => <ArchiveRecordCardShell key={record.id} metaText={formatDate(record.record_time, language)} mobileMode>
        {record.images.length ? <div className={`photo-grid ${record.images.length === 1 ? "single-photo" : ""}`}>{record.images.map((image) => <button type="button" className="photo-view" key={image.id} aria-label={language === "zh" ? "????" : "View photo"} onClick={() => setLightbox(image)}><BlobImage image={image} alt="" /></button>)}</div> : null}
    ...
    {lightbox ? <dialog className="photo-lightbox" open aria-label={language === "zh" ? "??" : "Photo"} onCancel={() => setLightbox(null)}><button type="button" className="icon-button" autoFocus onClick={() => setLightbox(null)}>{copy.back}</button><BlobImage image={lightbox} alt="" /></dialog> : null}
  </>;
}
...
function RecordForm({ copy, archive, language, record, onCancel, onSaved }: {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!isoTime) throw new Error(language === "en" ? "Enter a valid local date and time." : "??????????????");
